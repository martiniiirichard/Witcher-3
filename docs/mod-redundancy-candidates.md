...
| Candidate | Resolution | Evidence |
| --- | --- | --- |
| Source-shell compatibility mods | Keep for now. | Fresh 2026-06-28 scan found several enabled folders with zero winning scripts, but most are not dead. `mod00AMMW3EEReduxNG139b` and `modNPCTargetingPriorityAndAxiiFixes` are source/reference shells after manual grafts; `modReduxW3EE`, `modshrinesofverna`, `modmanticor`, and reputation/economy folders still have non-script payload or intentional umbrella ownership. See `logs/audit-source-shell-compatibility-mods-2026-06-28.md`. |
