...
| `modE3HUDColorLoad` | Optional cosmetic. Keep unless the look is unwanted. | Fresh 2026-06-27 unpack confirmed it contains exactly ten `gameplay/gui_new/swf/loading/*.redswf` files, wins them over base `modE3HUD`, and has no scripts/menu/input surface. | Low script risk. Removal should mainly affect loading screen/color presentation. |
| `modAMM` + `mod00AMMW3EEReduxNG139b` | Keep as a pair while AMM is desired. | `modAMM` is the base Appearance Menu Mod layer; `mod00AMMW3EEReduxNG139b` is a narrow W3EE Redux compatibility script patch. Fresh 2026-06-27 audit confirmed `DLCAMM`, `DLCAMMHORSE`, `AMM.xml`, and `AMMRoach.xml` are present, and active AMM files contain manual grafts from Tailory, New Hairstyles, Shades of Iron, Swords on Roach, and W3EE Redux compatibility work. | Keeping AMM without the patch risks returning player script conflicts. Removing AMM loses appearance customization and dependent AMM content/grafts. |
| `modLiveBestiary` | Keep if the Live Bestiary feature is desired. | The data table and bundle remain active, while the original bestiary script is disabled so the E3/W3EE-compatible path can win. Fresh 2026-06-27 audit confirmed `DLCLiveBestiary\content\dlc\globals\lbTable.csv` exists, malformed loose DLC-root helper folders are absent, and `mod0_E3HUD_W3EER_NG` reads `LoadCSV( "dlc\globals\lbTable.csv" )`. | Removing it should reduce data/content footprint, but it likely removes the Live Bestiary feature contribution. |
| `modMenuOrganizerNG_IMM_MrCK` | Keep as infrastructure. | It owns menu organization scripts and strings for the current crowded mod menu stack. Fresh 2026-06-27 audit confirmed its XML is registered in both `dx11filelist.txt` and `dx12filelist.txt`, and the prior Menu Organizer XML error was fixed by that registration. | Casual removal can make the options menu harder to reason about or break menu registration again. Treat removal as a deliberate UI-stack redesign, not cleanup. |
